...
if [ "${TRAVIS_BRANCH}" = "main" ]; then
  if [ "${TRAVIS_PULL_REQUEST}" = "false" ]; then
    ...
    aws s3 cp s3://$S3_BUCKET_NAME/authorisation/production.env ./secrets.env

    STACK_NAME=authorisation
    ...
    aws s3 cp s3://$S3_BUCKET_NAME/authorisation/sys.env ./secrets.env

    STACK_NAME=authorisation-sys
